...
def predict_test(model, seq_times, seq_types, seq_lengths, pad, device='cpu',
    ...
    for index_ in index_range_:
        ...
        #print('est', est)
        #print('est type', type(est))
        incr_estimates.append(est.item())
        incr_real.append(real_dt.item())
        incr_errors.append(err.item())
        types_real.append(real_type.item())
        types_estimates.append(est_type.item())
